...
num_acc_mon |>
  ...
  labs(title = "December has the Most Average Number of Accidents in 2021, San Diego County",
       x = "Number of Accidents") +
  theme_minimal() +
  theme(axis.title.y = element_blank(),
        plot.title.position = "plot",
...
```{r}
top_20_dates <- head(df$Date, 20)
crash_filtered <- crash %>% filter(Date %in% top_20_dates)

crash_relationships <- lm(Daily_total ~ Weather_category + Precipitation + Visibility + Pressure + Humidity + Wind_chill + Temperature + Season, data = crash_filtered)

summary(crash_relationships)
```

To evaluate the effectiveness of our model in predicting the variability in daily total accidents, we will use a backward selection approach. The goal of this approach is to identify the most significant predictors in the model and eliminate any non-significant predictors. 

After applying this method and comparing the resulting R-squared value with the original model, we found that the highest R-squared value was similar to the one obtained from our original model. Therefore, we will continue to use the original model in our analysis.

```{r}
back_ward <- ols_step_all_possible(crash_relationships) |>
  arrange(desc(adjr))
head(back_ward)
```
...
The "Foggy" and "Stormy" categories in the “Weather_category” variable have negative coefficients, indicating that crashes are less likely to occur in these weather conditions compared to the "Fair" weather condition. This was an interesting result - that car accidents are more likely to occur in fair weather. This could be attributed to the fact that drivers are generally more cautious in riskier weather, and could potentially be looked into in a further extension of this study.

The coefficients for the continuous variables indicate the change in "Daily_total" for a one-unit increase in each predictor variable, holding all other variables constant. For example, a one-unit increase in "Precipitation" is associated with an increase of 225.92837 in "Daily_total".
For seasons, "Winter" as the reference category and coefficient for "Spring" is negative, indicating that crashes are less likely to occur in spring compared to winter. This aligns with the plots we have done in Question 1 and our EDAs.
...
The multiple R-squared value is 0.4328, which means that about 43% of the variability in the daily total can be explained by the independent variables included in the model.

The F-statistic measures the overall significance of the model, i.e., whether the independent variables as a group are significantly related to the dependent variable. The F-statistic value is 246.5, which is significant (p-value < 2.2e-16), indicating that the independent variables included in the model are jointly significant in explaining the variation in the daily total.
