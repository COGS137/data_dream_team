...
Based on the most recent projections by the National Highway Traffic Safety Administration, a staggering 31,785 individuals lost their lives in traffic crashes during the first nine months of 2022. Shockingly, roughly one in every ten of these fatalities occurred in California. As UCSD students, we are frequently driving around the city and are thus both curious and concerned about the factors contributing to these accidents. With recent weather patterns growing increasingly unpredictable, we are eager to investigate whether there exists a correlation between weather and the frequency of accidents. Our study aims to explore this potential relationship between weather conditions, seasons, and the daily frequency of accidents in San Diego. By carefully analyzing traffic data, we hope to uncover whether weather and seasonal changes hold a significant impact on the frequency of accidents in our area. Ultimately, our goal is to provide insights that may aid in improving road safety and reducing the number of accidents in “America’s Finest City”.
...
All together, our finalized and wrangled dataset included 23,915 accidents from San Diego County in 2021 and these accidents each included 39 descriptor variables. 
...
#### Filter For San Diego County and 2021

First, we started off by filtering for San Diego County and crashes that happened in 2021. We will be saving our wrangled dataset by the name of "crash". 

```{r, eval = FALSE}
crash <- crash |>
  filter(County == "San Diego", Year == 2021)
```
...
Upon further examination of the original "Weather_Condition" values, we observed 56 unique weather values. The values ranged from volcanic ash, wintry mix, mostly cloudy, dust storm, to thunder storms. This posed a problem for our second question, because without consistent weather values, we believed that we may not be able to make conclusive decisions about the relationship between accident frequencies and weather conditions. To make this variable useful, in the context of our analysis, we took a thorough look at each and every unique value and then determined groupings of "Weather_categories" that would best fit each value. To start, we made a lookup table with our desired categories and keywords that would serve as our values to be checked. 
...
ggplot(crash_A, aes(x=Day, y=Daily_total)) +
  ...
  labs(title = "Distribution of number of accidents accross months 2021, San Diego County",
       subtitle = "More accidents occured at the end of the year",
...
crash_A |>
  ...
  labs(title = "December 14th had the most accident in 2021 across days, San Diego County",
       x = "Day") +
  theme_minimal() +
...
ggplot(crash, mapping = aes(x = Season, fill = Weather_category)) + 
  ...
  labs(title = "Relationship between Seasons and Weather Conditions", 
       x = "Season", y = "Count", fill = "Weather Conditions") +
  theme(plot.title.position = "plot") +
  theme_minimal()
```

#### Daily Total Accidents Broken Down By Weather Conditions

After the aforementioned visualizations has taken place, we then move on to observing the relationship between the number of total daily accidents with weather categories, noticing that fair is the outlier when it comes to accidents broken down by weather category.
...
#### Distribution of Accidents by Season

In the same vein we also check the relationship between seasons and observe that Winter and Fall are the seasons in which the most amount of accidents occur.

```{r}
ggplot(crash, mapping = aes(x = Daily_total, y = fct_infreq(Season))) + 
...
#### ScatterPlot for Total Daily Accidents by Season And Date in 2021

Finally, we create a scatterplot where we observe total daily accidents by date and season, again recognizing that winter and fall have the greatest peaks in total daily accidents. This causes us to wonder what these peaks may be correlated with, perhaps there is more travel during these seasons, or maybe weather conditions play a role. 
...
ggplot(crash, mapping = aes(x = Date, y = Daily_total, color = Season)) +
  ...
  labs(x = "Date", y = "Total Accidents", title = "Accidents by Date and Season", 
       color = "Season") +
...
num_acc |>
  ...
  labs(title = "December 14th had the most accident in 2021, San Diego County",
...
num_acc_mon |>
  ...
  labs(title = "December has the most accident in 2021, San Diego County",
       x = "Number of Accidents") +
  theme_minimal() +
  theme(axis.title.y = element_blank(),
        plot.title.position = "plot",
...
num_acc_mon |>
  ...
  labs(title = "December has the most average number of accident in 2021, San Diego County",
       x = "Number of Accidents") +
  theme_minimal() +
  theme(axis.title.y = element_blank(),
        plot.title.position = "plot",
